...
const AppSidebar = () => {
  ...
  // Build profile URL dynamically, fallback to "/login" if no user
  const profileUrl = currentUser?.id ? `/profile/${currentUser.id}` : "/login";
        ...
        <div className="space-y-3 pt-2 border-t border-border">
          ...
          <NavLink to="/profile">
            <Button
              variant="ghost"
              size="sm"
              className={`w-8 h-8 p-0 transition-smooth ${getNavClass(currentUser?.id ? `/profile/` : "/login")}`}
              title="Profile"
            >
              <User size={16} />
            </Button>
