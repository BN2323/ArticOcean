...
exports.getProfile = async (req, res) => {
  try {
    const userId = req.params.id || req.user.id; // use ID from params or current user
    ...
    userJSON.isFollowed = user.followers.some(follower => follower.id === req.user.id);
    userJSON.articles.forEach(article => {
      delete article.likers;
      delete article.bookmarkers;
    });
...
exports.getMyProfile = async (req, res) => {
  try {
    const userId = req.user.id; // use ID from params or current user
    console.log("Fetching profile for user ID:", userId);
